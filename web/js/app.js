...
function runBfs() {
  ...
  state.inaccessible = [];
  $("output").innerHTML = `<div class="compare-item"><h3>BFS · fewest stops · O(V+E)</h3><div class="path-line">${formatPath(r)}</div></div>`;
  drawMap();
}
...
function runDijkstra() {
  ...
  state.highlight = r ? r.path : [];
  state.inaccessible = [];
  $("output").innerHTML = `<div class="compare-item"><h3>Dijkstra · shortest distance · O((V+E) log V)</h3><div class="path-line">${formatPath(r)}</div></div>`;
  drawMap();
}
...
function runDfs() {
  ...
  $("output").innerHTML = `<div class="compare-item"><h3>DFS · all paths (max 20) · backtracking</h3><ol>${items}</ol></div>`;
  drawMap();
}
...
function runCompare() {
  ...
  if (bfs && dijkstra && bfs.distance !== dijkstra.distance) {
    summary = `BFS minimises stops (${bfs.stops}). Dijkstra minimises metres (${dijkstra.distance}m vs ${bfs.distance}m).`;
  }
  $("output").innerHTML = `
...
function demoBfsVsDijkstra() {
  state.start = "210 EXTC Lab-8";
  state.end = "213 Computer Centre";
  $("start").value = state.start;
  $("end").value = state.end;
  state.profile = "none";
  $("profile").value = "none";
  $("profileNote").textContent = PROFILES.none.note;
  runCompare();
  showStatus("Viva demo: C-Wing lab chain. BFS uses the corridor (2 stops). Dijkstra walks 210→211→212→213 (shorter metres).");
}

function demoWheelchair() {
  state.start = "Central Junction";
  state.end = "Auditorium Hall";
  state.profile = "wheelchair";
  $("start").value = state.start;
  $("end").value = state.end;
  $("profile").value = "wheelchair";
  $("profileNote").textContent = PROFILES.wheelchair.note;
  runCompare();
  showStatus("Viva demo: wheelchair to Auditorium uses B-Wing Fire Lift. Stairs are skipped. Block that lift to make the hall unreachable.");
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  $("btnDemo1").addEventListener("click", demoBfsVsDijkstra);
  $("btnDemo2").addEventListener("click", demoWheelchair);
